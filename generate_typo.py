...
TOTAL_TYPO_TYPE = [['ㄲ', 'ㅆ', 'ㅃ', 'ㅉ', 'ㄸ'],
                   ['ㅒ', 'ㅖ'],
                   ['ㄱ', 'ㅅ', 'ㅛ', 'ㅕ', 'ㄹ', 'ㅎ', 'ㅗ', 'ㅊ', 'ㅍ', 'ㅠ', 'ㄺ', 'ㄽ'],
                   ['ㅂ', 'ㅈ', 'ㄷ', 'ㅁ', 'ㄴ', 'ㅇ', 'ㅋ', 'ㅌ', 'ㅑ', 'ㅐ', 'ㅔ', 'ㅓ', 'ㅏ', 'ㅣ', 'ㅜ', 'ㅡ',
                    'ㅘ', 'ㅚ', 'ㅟ', 'ㅢ']]
            ...
            if pro >= len(type_list):  # 정해진 오타 개수 유지를 위해서는 수정해야함. 선정한 오타 타입의 오타 개수보다
                ...
                while (n > 0):
                    ...
                    if type_list[pos] not in word_pos:
                        ...
                        check_list[type_list[pos]] = np.zeros(len(pro_list), dtype=int)
                        result_pro_list[i] += 1
                        n -= 1
...
def make_typo(sentence, typo_type_pro_list, n_typo):
    ...
    for i, t in enumerate(result_type_list):
        if t > 0:
            for j in range(t):
                typo_words.append(join_char_list(word_typo(words[typo_pos][type_num], TOTAL_TYPO_TYPE[i])))
                type_num += 1
...
def main():
    ...
    for i, line in enumerate(lines):
        sent_list, typo_pos= make_typo(line.strip(), TYPO_TYPE_PRO_LIST, args.per_sent_typo_num)
